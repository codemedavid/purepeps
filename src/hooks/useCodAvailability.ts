  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // An absent row means the setting was never seeded — treat as enabled,
        // matching the migration's default.
        const raw = data?.value;
        setCodEnabled(raw == null ? true : ['true', '1', 'yes'].includes(String(raw).trim().toLowerCase()));
      } catch (err) {
